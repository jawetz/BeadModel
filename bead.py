...
class Bead:
    def __init__(self, pos, theta, disp, w, R, r, thtb,mis):
        self.pos = pos
        self.theta = theta
        self.disp = disp
        self.w = w
        ...
        self.thtb=thtb
        self.mis=mis
        w_adj=(self.R-self.r)*(np.tan(np.pi/2-self.thtb+self.mis)-np.tan(np.pi/2-self.thtb))
        ...
        self.C = np.array(self.D) - np.array([(self.w-w_adj) * np.cos(-self.theta), (self.w-w_adj) * np.sin(-self.theta)])
        self.E = np.array(self.F) - np.array([(self.w-w_adj) * np.cos(-self.theta), (self.w-w_adj) * np.sin(-self.theta)])
